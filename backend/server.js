...
// 测试中文文章创建
app.get('/api/test-chinese', async (req, res) => {
  const pool = getPool();
  ...
  try {
    ...
    const content = JSON.stringify({ 
      sentences: [
        { text: 'こんにちは', meaning: '你好' },
        { text: 'ありがとう', meaning: '谢谢' }
      ] 
    });
    // 构建标签字符串
    const tagsString = '测试,中文';

    // 插入文章
    await pool.execute(
      'INSERT INTO articles (id, title, content, tags) VALUES (?, ?, ?, ?)',
      [id, '测试文章', content, tagsString]
    );
